...
.career{display:grid;grid-template-columns:repeat(auto-fit,minmax(135px,1fr));gap:.5rem;margin-top:.9rem}
.career h4{grid-column:1/-1;margin:.6rem 0 0;font-family:var(--fm);font-size:.7rem;letter-spacing:.16em;text-transform:uppercase;color:var(--scarlet)}
.career h4:first-child{margin-top:0}
.career div{background:var(--panel-2);border:1px solid var(--rule-2);border-radius:6px;padding:.55rem .7rem;min-width:0}
.career dt{font-family:var(--fm);font-size:.66rem;letter-spacing:.12em;text-transform:uppercase;color:var(--ink-3)}
.career dd{margin:.15rem 0 0;font-family:var(--fd);font-variation-settings:"wdth" 110,"wght" 800;font-size:1.15rem;line-height:1.1}
.career dd small{display:block;font-family:var(--fb);font-weight:400;font-size:.76rem;color:var(--ink-2);margin-top:.15rem}
...
<div class="modal" id="career" role="dialog" aria-modal="true" aria-labelledby="careerT"><div class="mbox wide">
<button class="x" id="careerX" aria-label="Close">&times;</button>
<h2 id="careerT">Career <span>stats</span></h2>
...
function achCtx(p){
  ...
  const beatDivs={}; Object.keys(p.opp).forEach(o=>{ const q=byN[o]; if(q&&q.d&&p.opp[o][0]>0) beatDivs[q.d]=1 });
  const nemesisBeaten=Object.keys(p.opp).some(o=>p.opp[o][0]>=1&&p.opp[o][2]>=4);
  let debuts=0; const seenDebut={};
  p.log.forEach(l=>{ const q=byN[l.o];
...
function achX(p){
  ...
  // beating up
  x.maxGap=0; x.winsHigher=0;
  L.forEach(l=>{ if(l.s===1){ const g=l.orat-l.mrat; if(g>x.maxGap) x.maxGap=g; if(g>0) x.winsHigher++ } });
  x.dragon=false;
  nis.forEach(ni=>{ const N=NIGHT[ni]; if(!N||!N.table.length) return;
    let top=null, tr=-1; N.table.forEach(r=>{ const q=byN[r[0]]; if(!q) return; const rr=q.rb[ni]!=null?q.rb[ni]:q.seed; if(rr>tr){ tr=rr; top=r[0] } });   // rating going into that night
  ...
  x.settled=false; x.revenge=false; x.turned=false; x.deadEven=false; x.lead=0; x.beatUnique=0; x.drewTwice=false; x.maxWith=0;
  Object.keys(seq).forEach(o=>{
    ...
    if(w+d+l>=4&&w===l) x.deadEven=true;
    if(w-l>x.lead) x.lead=w-l;
    if(w>0) x.beatUnique++;
  ...
  nis.forEach(ni=>{
    ...
    if(mine&&mine.delta>0&&!P.some(q=>q.n!==p.n&&q.nightly.some(n=>n.ni===ni&&n.delta>mine.delta))) x.mainCharacter=true;
  ...
  x.comeback=false;
  for(let i=1;i<nis.length;i++){ if(daysBetween(DATES[nis[i-1]],DATES[nis[i]])>=90&&byNi[nis[i]][0].s===1) x.comeback=true }
  x.plotArmor=false;
  ...
  x.nightGames=nis.map(ni=>byNi[ni].length);   // this season, in order, for the exact-number secrets
  x.seasonWins=p.rec[0]; x.seasonGain=p.hist.length?p.r-p.hist[0][1]:0;
  x.positive=p.games>=10&&p.rec[0]>p.rec[2];
...
function achDetail(p,id){
  ...
  let status="";
  if(r.got){ const w=achWhen(p)[id]; status=w==="earlier"?"Earned before this season.":w?"Earned "+fd(w)+".":"Earned." }
...
function achToggle(tile){
  const p=CUR, box=$("#achInfo"); if(!p||!box) return;
...
function facts(p){
  ...
  if(p.games>=2) c.push([surprise(function(x){return x.games},p.games)-.1,"♜",p.games,"Games played in "+seaLower+"."]);
...
function recentGames(p){
  ...
  return '<div class="rg-wrap"><table class="rg"><tbody>'+p.recent.map(function(l){ var n=nd[l.ni], r=res(l);
    return '<tr><td class="d">'+fshort(DATES[l.ni])+'</td><td class="o" data-o="'+esc(l.o)+'" tabindex="0" role="button"><i class="cc c'+l.c+'"></i>'+esc(anon(l.o))+(l.s===1&&l.orat-l.mrat>=200?'<i class="zap" title="Upset alert: '+(l.orat-l.mrat)+' points up">&#9889;</i>':'')+'<span style="font-family:var(--fm);font-size:.7rem;color:var(--ink-3)"> '+l.orat+'</span></td>'+
...
/* Everything the club knows about one player, in one place: this season, this
   era before it, and Seasons 1-7 on the old system. */
function careerPanel(p){
  const c=p.c||{}, out=[];
  const H=t=>out.push('<h4>'+t+'</h4>'), S=(k,v,n)=>out.push('<div><dt>'+k+'</dt><dd>'+v+(n?'<small>'+n+'</small>':'')+'</dd></div>');
  const rec=c.rec||p.rec, games=c.games||p.games, wins=rec[0], dr=rec[1], loss=rec[2];
  const pct=games?Math.round((wins+dr*0.5)/games*100):0;
  H("Every night on record");
  S("Games",games); S("Record",wins+"&ndash;"+dr+"&ndash;"+loss,"W&ndash;D&ndash;L"); S("Score",pct+"%","wins plus half the draws");
  S("Nights",c.nights!=null?c.nights:p.cons); if(c.first) S("First night",fd(c.first)); if(c.last) S("Latest night",fd(c.last));
  if(c.span) S("At the club",(c.span/365).toFixed(1)+" years",(c.months||0)+" different months");
  H("As White and Black");
  const wh=c.wh||p.wh, bl=c.bl||p.bl;
  S("As White",wh[0]+"&ndash;"+wh[1]+"&ndash;"+wh[2],(wh[0]+wh[1]+wh[2])+" games"); S("As Black",bl[0]+"&ndash;"+bl[1]+"&ndash;"+bl[2],(bl[0]+bl[1]+bl[2])+" games");
  H("Turning up");
  if(c.bestRun) S("Nights in a row",c.bestRun,"best run"); if(c.full!=null) S("Five-game nights",c.full); if(c.sweeps!=null) S("Clean sweeps",c.sweeps,"won every game, four or more");
  if(c.maxNight) S("Most games in a night",c.maxNight); if(c.share!=null) S("Nights made",Math.round(c.share*100)+"%","of the nights since your first");
  H("People");
  S("Different opponents",c.opps!=null?c.opps:Object.keys(p.opp).length); if(c.topRival) S("Most games with one person",c.topRival);
  H("This season");
  S("Rating",p.r,"&plusmn;"+p.rd); S("Record",p.rec[0]+"&ndash;"+p.rec[1]+"&ndash;"+p.rec[2],p.games+" games"); S("Nights",p.cons+" of "+DATES.length);
  if(p.peak) S("Season high",p.peak[0],fshort(p.peak[1])); if(p.streak&&p.streak[0]) S("Best winning run",p.streak[0]+" straight");
  S("Cups",p.trophies[0]+" &middot; "+p.trophies[1]+" &middot; "+p.trophies[2],"gold &middot; silver &middot; bronze");
  if(p.v&&p.v[3]){ H("This era, before this season"); S("Games",p.v[3],VAULT?fshort(VAULT.from)+" &ndash; "+fshort(VAULT.to):""); S("Record",p.v[0]+"&ndash;"+p.v[1]+"&ndash;"+p.v[2]) }
  const old=ARCBY[p.n]||(ARC&&ARC.link&&ARC.link[p.n]?ARCBY[ARC.link[p.n]]:null);   // the archive is keyed by the current name
  if(old){ H("Seasons 1&ndash;7, the old system"); S("Games",old.g); S("Record",old.rec[0]+"&ndash;"+old.rec[1]+"&ndash;"+old.rec[2]); S("Old rating",old.r,"not comparable with today's") }
  return '<dl class="career">'+out.join("")+'</dl>';
}
let careerOpener=null;
function openCareer(p){ careerOpener=document.activeElement; $("#careerT").innerHTML=E(p.n)+' <span>career</span>'; $("#careerBody").innerHTML=careerPanel(p); $("#career").classList.add("open"); $("#careerX").focus() }
function closeCareer(){ const was=$("#career").classList.contains("open"); $("#career").classList.remove("open"); if(was&&careerOpener){ careerOpener.focus(); careerOpener=null } }
$("#careerX").onclick=closeCareer;
$("#career").onclick=e=>{ if(e.target===$("#career")) closeCareer() };
document.addEventListener("keydown",e=>{ if(e.key==="Escape") closeCareer() });
...
function comparePanel(p){
  ...
  return sel+'<div class="vsbar"><div class="side">'+av(p.n,"m")+'<b>'+esc(p.n)+'</b><span>'+p.r+'</span><em>this page</em></div>'+
   ...
   (a?'<p style="text-align:center;font-family:var(--fm);font-size:.7rem;color:var(--ink-3);margin:-.6rem 0 1rem">'+(a[1]?a[1]+' drawn &middot; ':'')+'last met '+fshort(DATES[lastMeet.ni])+', '+esc(p.n)+' '+(lastMeet.s===1?"won":lastMeet.s===.5?"drew":"lost")+'</p>':'')+body;
}
...
function showProfile(n,rival){
  ...
  $("#ptro").outerHTML=trophyHead(p);
  $("#psub").innerHTML='<span>'+esc(longDiv(p.d))+'</span><span>'+p.games+' games</span>'+(p.rd>RS?'<span class="pill set">still settling · ±'+p.rd+'</span>':'')+(away(p)?'<span class="pill">away · '+awayWhy(p)+'</span>':'');
  $("#pact").innerHTML='<button class="chip" id="careerBtn">Career stats</button><button class="cmpbtn" id="cmpGo">Compare with a rival</button>'+suggestRivals(p).map(function(r){return '<button class="chip" data-r="'+esc(r[0])+'"><small>'+r[1]+'</small>'+esc(r[0])+'</button>'}).join("")+
  ...
  $("#careerBtn").onclick=function(){ openCareer(p) };
  ...
  $("#facts").innerHTML=facts(p).map(function(f){return '<div class="fact"><div class="ic">'+f[0]+'</div><div class="ft"><b>'+f[1]+'</b><span>'+f[2]+'</span></div></div>'}).join("");
  drawGraphs();
...
document.addEventListener("click",function(e){
  ...
  if(bh&&CUR){
    const why=oddNightWhy(CUR,parseInt(bh.dataset.bh,10));
