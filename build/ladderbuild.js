...
function openingEggs(p){
  ...
  if(top&&top.n===p.n&&lastN&&lastN.l>0&&Math.random()<0.7){
    setTimeout(function(){
      egg("The bigger they are, the harder they fall. Or something like that.",4000,null,"mighty-fall");
    },1500);
    return;
  }
  if(p.rec[0]>p.rec[2]&&p.games>=10&&Math.random()<0.15){
    setTimeout(function(){
      egg(WINNERS[Math.floor(Math.random()*WINNERS.length)],3400,null,"winning-record");
    },1500);
  }
}
