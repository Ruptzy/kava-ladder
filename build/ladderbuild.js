...
$(".wm small").onclick=()=>egg("Kava has been playing since September 2022. "+D.games.length+" games this era alone.");
// 3. the updated line owns up to what it knows
$("#upd").style.cursor="pointer";
$("#upd").onclick=()=>egg("Built "+fd(D.built)+" from "+D.games.length+" games over "+DATES.length+" nights. Nothing typed in by hand.");
// 4. tonight
...
$("#bt").onclick=function(){ ladIdx=(ladIdx+1)%LADNAMES.length;
  ...
  if(ladIdx===1) egg("Call it what you like.") };
...
function maybeVibes(){
  ...
  egg("You found the vibes. They are now sortable.",3200);
}
...
function profileEggs(p){
  ...
  if(rate){ rate.style.cursor="pointer"; rate.onclick=function(){
    ...
    egg("From "+from+" to "+to+", one game at a time.");
  } }
  ...
  if(av){ av.style.cursor="pointer"; av.onclick=function(){
    ...
    egg(p.n+" plays like a "+spiritOf(p.n).split(" ")[1]+".");
...
function searchEgg(v){
  ...
  if(k==="1337"||k==="leet"){ flash("gold-mode",2600); egg("Elite.",2000); return true }
  if(k==="checkmate"){ egg("Not on this page. Take it to the board.",2600); return true }
  return false;
}
...
(function(){
  ...
  document.addEventListener("keydown",function(e){
    ...
    if(i===seq.length){ i=0; flash("gold-mode",5000); egg("Tournament mode. Purely decorative.",3200) }
  });
})();
