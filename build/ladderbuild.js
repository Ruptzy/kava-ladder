...
@media(max-width:640px){
  ...
  .tprev,.ptro .cap2{text-align:center}.ptro .tset{justify-content:center}
  .ptro .it{min-width:0}
...
@media(min-width:641px) and (max-width:900px){
  ...
  .ptro .tset{justify-content:center}.tprev,.ptro .cap2{text-align:center}
...
function scopeDefs(){
  ...
  // the one place the two eras really are added together
  if(preEnd&&!(SEASON&&SEASON.no<first)) defs.push({k:"pre",label:"Seasons 1\u2013"+(first-1),sub:"old system + Season "+(first-1)});
...
function scopeOf(k){
  ...
  if(k==="pre"){ const first=ERAS.first||0, un={}; for(let i=0;i<O;i++) un[i]=1;
    const idx=[]; ERAS.season.forEach((s,i)=>{ if(s<first) idx.push(i) }); const inP=i=>idx.indexOf(i)>=0;
    const hist={}; Object.keys(ERAS.hist).forEach(n=>{ hist[n]=ERAS.hist[n].filter(h=>inP(h[0])).map(h=>[h[0]+O,h[1],h[2]]) });
