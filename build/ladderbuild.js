...
.tabs.art{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.5rem;padding:.2rem 0 .9rem}
.tabs.art button{position:relative;border:none;padding:0;background:transparent no-repeat center/100% auto;
aspect-ratio:1000/300;border-radius:8px;opacity:.55;filter:saturate(.75);transition:opacity .15s,filter .15s}
.tabs.art button:hover{opacity:.85;filter:none}
.tabs.art button[aria-selected=true]{opacity:1;filter:none;box-shadow:0 0 0 1px var(--scarlet-dim),0 8px 26px -12px var(--scarlet)}
.tabs.art button .lg,.tabs.art button .sm{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0)}
.tabs.art button .n{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0)}
.tabs.art button::after{content:"";position:absolute;left:12%;right:12%;bottom:-2px;height:2px;border-radius:2px;background:transparent}
.tabs.art button[aria-selected=true]::after{background:var(--scarlet)}
...
@media(max-width:640px){
  ...
  .tabs.art{grid-template-columns:repeat(2,minmax(0,1fr));gap:.4rem}
  .tabs.art .lg,.tabs.art .sm{display:none}
  .tabs.art button .n{font-size:.55rem}
...
<div class="tb"><img src="logo.png" alt="KAVA Social Chess Club" width="66" height="66">
...
<div class="sh" id="ladder"><h2 id="bt">Ladder</h2><p><span id="btCount"></span> &middot; tap a name</p></div>
...
$("#tabs").innerHTML=TABS.map(t=>{
  ...
  const bg=artOK?' style="background-image:url(tabs/'+tabSlug(t.i)+'.png)"':'';
  return '<button role="tab" data-d="'+E(t.i)+'" aria-selected="false" aria-label="'+E(longDiv(t.i))+', '+n+' players"'+bg+
    '><span class="lg">'+E(longDiv(t.i))+'</span><span class="sm">'+E(shortDiv(t.i))+'</span><span class="n">'+n+'</span></button>'}).join("");
...
function draw(){
  ...
  $("#btCount").textContent=pool().length+" players";
