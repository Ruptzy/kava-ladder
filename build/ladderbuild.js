...
#champs:empty,#firstNote:empty,#you:empty,#race:empty{display:none}
...
#champs a:hover{border-color:var(--scarlet)}
#champs a b{color:var(--ink-3);font-weight:400}
#champs a span i{font-style:normal;color:var(--scarlet);margin-right:.3rem}
...
.gwrap{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.6rem;margin-bottom:.6rem;align-items:stretch}
...
.ptro{margin-left:auto;padding-left:1.2rem;border-left:1px solid var(--rule-2)}
.ptro .tset{display:flex;gap:1.1rem}
.ptro .lb em{display:block;font-style:normal;color:var(--scarlet);letter-spacing:.14em;margin-top:.2rem}
.tprev{margin:.55rem 0 0;font-family:var(--fm);font-size:.7rem;letter-spacing:.1em;
text-transform:uppercase;color:var(--ink-3);text-align:right}
.ptro .it{text-align:center;min-width:96px;position:relative}
...
.ptro .pic{position:relative;display:block;width:104px;height:104px;margin:0 auto .3rem;
...
font-size:2rem;line-height:1;color:var(--cream);text-shadow:0 2px 10px rgba(0,0,0,.6)}
...
.recs{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:.7rem;margin-bottom:1.2rem}
/* a season a card: champions, then its size */
.seasons{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:.7rem;margin-bottom:1.2rem}
.season-card{background:rgba(22,23,25,.94);border:1px solid var(--rule-2);border-radius:6px;padding:.9rem 1rem}
.season-card h3{margin:0 0 .5rem;font-family:var(--fd);font-variation-settings:"wdth" 112,"wght" 800;font-size:1.15rem}
.season-card h3 small{display:block;font-family:var(--fm);font-size:.7rem;letter-spacing:.14em;text-transform:uppercase;color:var(--ink-3);font-weight:400;margin-top:.15rem}
.season-card .ch{display:flex;gap:.6rem;align-items:baseline;padding:.3rem 0;border-bottom:1px solid var(--rule);font-size:.92rem}
.season-card .ch i{font-style:normal;color:var(--scarlet)}.season-card .ch b{flex:1}.season-card .ch span{font-family:var(--fm);font-size:.8rem;color:var(--ink-3)}
.season-card .sz{font-family:var(--fm);font-size:.72rem;letter-spacing:.08em;color:var(--ink-3);margin:.6rem 0 0;text-transform:uppercase}
.season-card a.howbtn{margin-top:.7rem}
.season-card.now{border-color:var(--scarlet-dim)}
...
<main id="rv" class="hid">
<button class="back" id="rbk">&larr; Back to the ladder</button>
<div class="sh"><h2>Past <span>seasons</span></h2><p>Who won what</p></div>
<div class="seasons" id="seasons"></div>
<div class="sh"><h2>All-time <span>records</span></h2><p>Every night on record</p></div>
<dl class="recs" id="recsAll"></dl>
<div class="sh"><h2 id="recsNowH">This <span>season</span></h2><p>So far</p></div>
...
function route(){
  ...
  if(parts[0]==="records"){ showRecords(); return }
  let i=0, nd="all", ns="r";
...
(function(){
  ...
  { const arch=D.arch||null, now=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10);   // the club's date, not UTC's
    ...
    // last season's champions, by the finale's own rule; the line opens that season
    { const c=$("#champs"), ch=(D.champs||[])[0], PIECE=["&#9818;","&#9820;","&#9823;"];
      if(c&&ch&&!arch&&ch.list.length) c.innerHTML='<a href="season-'+ch.no+'.html"><b>&#127942; Season '+ch.no+' champions</b>'+
        ch.list.map(x=>'<span><i>'+(PIECE[D.divisions.indexOf(x.d)]||"")+'</i>'+E(x.n)+'</span>').join("")+'</a>'; }
...
function rivalsChart(p){
  ...
  return rows.map(function(r){ var w=Math.abs(r[5])/mx*50;
    ...
    return '<div class="riv"'+(tap?' data-o="'+esc(r[0])+'" tabindex="0" role="button" style="cursor:pointer"':'')+'><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(r[0])+'</span>'+
...
function trophyHead(p){
  ...
  const top=p.topDiv||p.d, counts=p.byDiv[top]||[0,0,0];
  const any=counts[0]+counts[1]+counts[2];
  const tip=any ? "Top-three finishes on a club night in "+longDiv(top)
                : "No top-three finish yet in "+longDiv(p.d);
  const cups='<div class="tset">'+L.map(function(lab,i){
    return '<div class="it'+(counts[i]?'':' z')+'"><span class="pic t'+(i+1)+'"></span>'+
      ...
      '<span class="lb"><i>\u2013</i>'+lab+'<i>\u2013</i><em>'+esc(shortDiv(any?top:p.d))+'</em></span></div>';
  }).join("")+'</div>';
  // anything won in a lower bracket, before they climbed
  const rest=Object.keys(p.byDiv).filter(d=>d!==top).sort((a,b)=>divRank(a)-divRank(b))
    .map(d=>{ const c=p.byDiv[d];
      return esc(shortDiv(d))+" "+[0,1,2].filter(i=>c[i]).map(i=>c[i]+"\u00d7"+["1st","2nd","3rd"][i]).join(", ") });
  return '<div class="ptro" id="ptro" title="'+esc(tip)+'">'+cups+
    (rest.length?'<p class="tprev">Before that: '+rest.join(" \u00b7 ")+'</p>':'')+'</div>';
}
...
function vsTable(p){
  ...
  var lab='text-align:right;padding:0 .2rem .35rem .6rem;font-family:var(--fm);font-size:.7rem;font-weight:400;letter-spacing:.1em;text-transform:uppercase;color:var(--ink-3)';
  ...
  return '<table style="font-size:.85rem"><thead><tr><th style="padding:0"></th>'+
    ...
    opps.map(function(o){ var a=rec[o], k=a[0]>a[2]?"var(--gain)":a[0]<a[2]?"var(--loss)":"var(--ink-2)";
      ...
      return '<tr'+(gh||lump?'':' data-o="'+esc(o)+'" tabindex="0" role="button" style="cursor:pointer"')+'><td style="padding:.6rem .2rem">'+esc(o)+
        (gh?'<span style="font-family:var(--fm);font-size:.7rem;color:var(--ink-3);letter-spacing:.1em"> VISITOR</span>':'')+'</td>'+
...
function scopeDefs(){
  ...
  nos.sort((a,b)=>b-a);
  const defs=nos.map(no=>{ const cur=SEASON&&no===SEASON.no, ds=ERAS.dates.filter((d,i)=>ERAS.season[i]===no);
    return {k:"s"+no, label:"Season "+no, sub:cur?(D.arch?"final":"now"):(ds.length?fmy(ds[0])+" \u2013 "+fmy(ds[ds.length-1]):"")} });
  if(ERAS.old&&ERAS.old.dates.length) defs.push({k:"old",label:"Seasons 1\u20137",sub:"old system"});
  defs.push({k:"all",label:"Whole career",sub:"everything on record"});
  return defs;
}
function scopeOf(k){
  ...
  const bandFull=(name,ni,rIn)=>{ const no=ERAS.season[ni], bm=ERAS.bands[name];
    ...
    return s||bandOf(rIn) };
  let S;
  if(k==="old"){ const un={}; ERAS.old.dates.forEach((d,i)=>un[i]=1); const oR={}; P.forEach(p=>{ const r=oldRating(p.n); if(r!=null) oR[p.n]=r });
    ...
    S={key:k,label:"Whole career",lower:"their whole career",dates:ERAS.old.dates.concat(F),
      ...
      byes:ERAS.byes.map(b=>[b[0]+O,nm(b[1]),b[2]]),hist:hist,seed:ERAS.seed,unrated:un,
...
function kpiHTML(p){
  ...
  else rating='<div><dt>Rating</dt><dd>'+(p.hist.length?p.r+'<small>at the end</small>':'&mdash;<small>no games</small>')+'</dd></div>';
  return rating+
...
function scopeBar(p){
  ...
  const show=defs.filter(d=>isCurScope(d.k)||d.k==="all"||d.k===SCOPE||scoped(p,d.k).games>0);
  $("#scopeSel").innerHTML=show.map(d=>{ const sp=scoped(p,d.k);
    return '<option value="'+d.k+'"'+(d.k===SCOPE?' selected':'')+'>'+d.label+(d.sub?' \u00b7 '+d.sub:'')+' \u00b7 '+sp.games+' games</option>' }).join("");
  const sp=scoped(p,SCOPE), S=dsOf(sp);
  $("#scopeNote").textContent=sp.games?(sp.cons+" of "+S.dates.length+" nights"+(sp.first&&sp.last&&sp.first!==sp.last?" \u00b7 "+fmy(sp.first)+" \u2013 "+fmy(sp.last):"")):"No games in "+S.lower+".";
  bar.classList.remove("hid");
  $("#scopeSel").onchange=function(){ SCOPE=this.value; renderScope(CUR); scopeBar(CUR) };
}
...
function showProfile(n,rival){
  ...
  scopeBar(p);
  renderScope(p);
  $("#board").classList.add("hid"); $("#hv").classList.add("hid"); $("#rv").classList.add("hid"); $("#pv").classList.remove("hid");
...
function drawGraphs(){
  ...
  $("#graphs").innerHTML=
    ...
    card("Recent form", formStrip(p), "half")+
...
{ let seq=[], pawnTaps=0, pawnAt=0;
  document.addEventListener("click",e=>{
    const el=e.target.closest("#champs i, .rc h4 i, .rr i"); if(!el) return;
...
function eggHalloween(d){ if(d.getMonth()===9&&d.getDate()>=24){ eggFind("halloween");
  const swap=()=>document.querySelectorAll("#champs i, .rc h4 i, .rr i").forEach(i=>{ if(Math.random()<0.3&&!i.dataset.k){ i.dataset.k=i.textContent; i.textContent="\uD83D\uDC80" } });
...
const EGG_IDS=["badge","seasons","updated","ladder-name","wiggle","away","vibes","self-cell",
 ...
 "lenny","goblin","checkers","backgammon","pony","update","kava","leet","checkmate","records",
 // and thirty more
...
function searchEgg(v){
  ...
  if(k==="records"||k==="record"){ eggFind("records"); location.hash="#/records"; return true }
...
function seasonCard(s,now){
  const PIECE=["&#9818;","&#9820;","&#9823;"];
  const champs=(s.champs||[]).map(c=>'<div class="ch"><i>'+(PIECE[D.divisions.indexOf(c.d)]||"")+'</i><b>'+E(c.n)+'</b><span>'+E(longDiv(c.d))+' &middot; '+c.r+'</span></div>').join("");
  return '<div class="season-card'+(now?' now':'')+'"><h3>Season '+s.no+'<small>'+(now?'in progress &middot; ':'')+(s.from===s.to?'from '+fshort(s.from):fshort(s.from)+' &ndash; '+fshort(s.to))+'</small></h3>'+
    (champs||'<div class="ch"><b>'+(now?'Leaders so far':'No champions recorded')+'</b></div>')+
    '<p class="sz">'+s.nights+' night'+(s.nights===1?'':'s')+' &middot; '+s.games+' games &middot; '+s.players+' players</p>'+
    (now?'':'<a class="howbtn" href="season-'+s.no+'.html">Season '+s.no+' standings</a>')+'</div>';
}
function drawSeasons(){
  const box=$("#seasons"); if(!box) return;
  const cards=[];
  if(SEASON&&!D.arch){
    const leaders=D.divisions.map(d=>{ const p=rankedIn(d)[0]; return p?{d,n:p.n,r:p.r}:null }).filter(Boolean);
    const played=new Set(); D.games.forEach(g=>{ played.add(NAMES[g[1]]); played.add(NAMES[g[2]]) });
    cards.push(seasonCard({no:SEASON.no,from:DATES[0]||SEASON.from,to:DATES[LASTI]||SEASON.from,nights:DATES.length,games:D.games.length,
      players:[...played].filter(n=>byN[n]&&!byN[n].gh).length,champs:leaders},true));
  }
  (D.seasons||[]).forEach(s=>cards.push(seasonCard(s,false)));
  if(ARC&&ARC.seasons) cards.push('<div class="season-card"><h3>Seasons '+E(String(ARC.seasons))+'<small>'+fshort(ARC.from)+' &ndash; '+fshort(ARC.to)+' &middot; the old rating system</small></h3>'+
    '<div class="ch"><b>The club before this ladder</b></div><p class="sz">'+ARC.nights+' nights &middot; '+ARC.games+' games &middot; '+ARC.everPlayed+' players</p>'+
    '<button class="howbtn" id="seasonsHist">Club history</button></div>');
  box.innerHTML=cards.join("");
  const hb=$("#seasonsHist"); if(hb) hb.onclick=()=>{ location.hash="#/history" };
}
/* the all-time marks read the careers the build ships, which cover every night on record */
function drawAllTime(){
  const box=$("#recsAll"); if(!box) return;
  const rows=[]; const add=(label,value,note)=>rows.push('<div class="rec"><dt>'+label+'</dt><dd>'+value+(note?'<small>'+note+'</small>':'')+'</dd></div>');
  const M=P.filter(p=>!p.gh&&p.c);
  const best=(k,f)=>M.reduce((m,p)=>(!m||f(p)>f(m))?p:m,null);
  const g=best("games",p=>p.c.games); if(g) add("Most games",g.c.games,E(g.n));
  const w=best("wins",p=>p.c.rec[0]); if(w) add("Most wins",w.c.rec[0],E(w.n));
  const n=best("nights",p=>p.c.nights); if(n) add("Most nights",n.c.nights,E(n.n));
  const r=best("run",p=>p.c.bestRun); if(r&&r.c.bestRun>1) add("Most nights in a row",r.c.bestRun,E(r.n));
  const o=best("opps",p=>p.c.opps); if(o) add("Most different opponents",o.c.opps,E(o.n));
  const s=best("sweeps",p=>p.c.sweeps); if(s&&s.c.sweeps>0) add("Most clean sweeps",s.c.sweeps,E(s.n));
  const f=best("full",p=>p.c.full); if(f&&f.c.full>0) add("Most five-game nights",f.c.full,E(f.n));
  const tr=best("rival",p=>p.c.topRival); if(tr&&tr.c.topRival>1) add("Most games with one person",tr.c.topRival,E(tr.n));
  const nights=realNights();
  const busiest=nights.reduce((m,x)=>!m||x[1]>m[1]?x:m,null), turnout=nights.reduce((m,x)=>!m||x[2]>m[2]?x:m,null);
  if(busiest) add("Busiest club night",busiest[1]+" games",fd(busiest[0]));
  if(turnout) add("Best turnout",turnout[2]+" players",fd(turnout[0]));
  const y=best("years",p=>p.c.span); if(y) add("Longest at the club",(y.c.span/365).toFixed(1)+" years",E(y.n)+", since "+fd(y.c.first));
  box.innerHTML=rows.join("");
}
function drawRecords(){
  drawSeasons(); drawAllTime();
  { const h=$("#recsNowH"); if(h) h.innerHTML=(SEASON?"Season "+SEASON.no:"This season")+' <span>so far</span>' }
  const rows=[];
  const add=(label,value,note)=>rows.push('<div class="rec"><dt>'+label+'</dt><dd>'+value+
    (note?'<small>'+note+'</small>':'')+'</dd></div>');
  let up=null, big=null, run=null, rival=null, nightGames=null, climb=null;
  P.forEach(p=>{
    p.log.forEach(l=>{ const gap=l.orat-l.mrat;
      if(l.s===1&&(!up||gap>up.gap)) up={p:p.n,o:l.o,gap:gap,d:DATES[l.ni]} });
    p.nightly.forEach(n=>{
      const g=n.w+n.dr+n.l;
      if(!nightGames||g>nightGames.g) nightGames={n:p.n,g:g,d:n.d};
      if(!n.first&&(!climb||n.delta>climb.v)) climb={n:p.n,v:n.delta,d:n.d};
    });
    if(!run||p.streak[0]>run.v) run={n:p.n,v:p.streak[0]};
    Object.keys(p.opp).forEach(o=>{ const a=p.opp[o], g=a[0]+a[1]+a[2];
      if(!rival||g>rival.g) rival={a:p.n,b:o,g:g} });
  });
  const mostNights=P.reduce((m,p)=>!m||p.cons>m.cons?p:m,null);
  ...
  if(nightGames&&nightGames.g>0&&DATES.length>1) add("Most games in a night",nightGames.g,E(nightGames.n)+" on "+fd(nightGames.d));
  if(rival&&rival.g>1) add("Longest rivalry",rival.g+" games",E(rival.a)+" and "+E(rival.b));
  if(mostNights&&mostNights.cons>0&&DATES.length>1) add("Most nights",mostNights.cons+" of "+DATES.length,E(mostNights.n));
  if(mostGames&&mostGames.games>0&&DATES.length>1) add("Most games",mostGames.games,E(mostGames.n));
  ...
  $("#recs").innerHTML=rows.length?rows.join(""):'<p style="color:var(--ink-3);margin:0">Nothing to show until a couple of nights are in.</p>';
}
