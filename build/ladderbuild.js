...
#champs:empty,#firstNote:empty{display:none}
  ...
  { const arch=D.arch||null, now=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10);   // the club's date, not UTC's
    ...
    { const f=$("#firstNote"); let seen=false; try{ seen=localStorage.getItem("kv-first")==="1" }catch(e){}
      ...
      if(f&&!seen&&!arch&&!location.hash)
        f.innerHTML='<span>First time here? Tap your name in the table to see your page.</span><button id="firstX" aria-label="Got it">&times;</button>';
...
$("#jumpBtn").onclick=function(){
  ...
  setTimeout(function(){ const l=$("#ladder"); if(l) window.scrollTo({top:l.getBoundingClientRect().top+scrollY-8,behavior:"instant"}) },60);
};
