...
.ptro{display:flex;gap:1.1rem;margin-left:auto;padding-left:1.2rem;border-left:1px solid var(--rule-2)}
...
@media(max-width:640px){
  ...
  .ptro{margin:1rem 0 0;padding:.9rem 0 0;border-left:none;border-top:1px solid var(--rule-2);
  flex:1 1 100%;justify-content:space-around;gap:.3rem}
  .ptro .it{min-width:0}
...
function trophyHead(p){
  ...
  const tip=(p.trophies[0]+p.trophies[1]+p.trophies[2])
    ? "Top-three finishes on a club night in the "+longDiv(p.d)+" bracket"
    : "No top-three finish in the "+longDiv(p.d)+" bracket yet";
  return '<div class="ptro" id="ptro" title="'+esc(tip)+'">'+L.map(function(lab,i){
