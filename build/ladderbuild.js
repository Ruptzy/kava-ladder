...
function achX(p){
  ...
  Object.keys(seq).forEach(o=>{
    ...
    if(w+d+l>=5&&w===l) x.deadEven=true;
...
}
{
  ...
  X("higher-25","\uD83D\uDC06","Predator","Twenty-five wins over players rated above you.",(p,c)=>achX(p).winsHigher>=25,(p,c)=>[achX(p).winsHigher,25]);
  // rivalries
  X("rivalry-2","\uD83D\uDC4B","Hello Again","Play the same person twice.",(p,c)=>achX(p).maxWith>=2);
  X("rivalry-20","\u2694\uFE0F","Archrivals","Play the same person twenty times.",(p,c)=>achX(p).maxWith>=20,(p,c)=>[achX(p).maxWith,20]);
  ...
  X("dead-even","\u2696\uFE0F","Dead Even","A rivalry of five or more games, exactly level.",(p,c)=>achX(p).deadEven);
  X("dominance","\uD83E\uDDB8","Dominance","Lead someone by five wins head to head.",(p,c)=>achX(p).lead>=5,(p,c)=>[achX(p).lead,5]);
  X("boogeyman","\uD83D\uDC7B","Boogeyman","Lead someone by ten wins head to head.",(p,c)=>achX(p).lead>=10,(p,c)=>[achX(p).lead,10]);
  // the room
  ...
  X("cup-10","\uD83C\uDFC6","Cabinet Needed","Ten top-three finishes.",(p,c)=>c.podium>=10,(p,c)=>[c.podium,10]);
  X("cup-20","\uD83C\uDFC6","Cup Hoarder","Twenty top-three finishes.",(p,c)=>c.podium>=20,(p,c)=>[c.podium,20]);
  X("cup-30","\uD83C\uDFC6","Silverware Dynasty","Thirty top-three finishes.",(p,c)=>c.podium>=30,(p,c)=>[c.podium,30]);
  // a night
  X("untouched","\uD83E\uDDE4","Untouched","Finish a night of four or more games without losing.",(p,c)=>achX(p).untouched);
  X("marathon","\uD83C\uDFC3","Marathon Man","Play seven or more games in one night.",(p,c)=>achX(p).marathon);
  X("personal","\uD83D\uDE24","Tonight Was Personal","Beat the same person twice in one night.",(p,c)=>achX(p).personal);
  ...
  X("redemption","\uD83C\uDF05","The Redemption Arc","Lose to someone and beat them later the same night.",(p,c)=>achX(p).redemption);
  ...
  // secrets
  X("nice","\uD83D\uDE0F","Nice.","Sit on exactly 69 recorded games.",(p,c)=>c.games===69);
  X("average","\uD83D\uDE10","Perfectly Average","Land on a rating of exactly 1500.",(p,c)=>p.r===1500);
  X("the-answer","\uD83E\uDDE0","The Answer","Sit on exactly 42 recorded games.",(p,c)=>c.games===42);
  X("deja-vu","\uD83D\uDC08\u200D\u2B1B","D\u00E9j\u00E0 Vu","The same win-draw-loss line two nights running.",(p,c)=>achX(p).dejaVu);
  X("groundhog","\uD83E\uDDAB","Groundhog Day","The same opponents two nights running.",(p,c)=>achX(p).groundhog);
  X("uno-reverse","\uD83D\uDD03","Uno Reverse Card","Beat someone in the very next game after they beat you.",(p,c)=>achX(p).uno);
  X("you-again","\uD83D\uDE11","You Again?!","Three games in a row against the same person.",(p,c)=>achX(p).youAgain);
  X("immovable","\uD83E\uDEA8","The Immovable Object","Draw with the same person twice.",(p,c)=>achX(p).drewTwice);
...
function recentGames(p){
  ...
  return '<div class="rg-wrap"><table class="rg"><tbody>'+p.recent.map(function(l){ var n=nd[l.ni], r=res(l);
    return '<tr><td class="d">'+fshort(DATES[l.ni])+'</td><td class="o" data-o="'+esc(l.o)+'" tabindex="0" role="button"><i class="cc c'+l.c+'"></i>'+esc(anon(l.o))+'<span style="font-family:var(--fm);font-size:.7rem;color:var(--ink-3)"> '+l.orat+'</span></td>'+
